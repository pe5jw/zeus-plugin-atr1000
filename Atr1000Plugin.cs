...
public sealed class Atr1000Plugin : IZeusPlugin, IBackendPlugin
{
    ...
    public async Task InitializeAsync(IPluginContext context, CancellationToken ct)
    {
        ...
        // Subscribe to rig frequency if ReadRadioState was granted.
    ...
    public async Task ShutdownAsync(CancellationToken ct)
    {
        ...
        if (_ctx?.Radio is { } radio && _freqHandler is { } h)
            radio.FrequencyChanged -= h;
    ...
    public void MapEndpoints(IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost("options", SetOptions);       // { "autoTune": true }

        endpoints.MapPost("tune", Tune);                 // { "mode": "memory|full|fine|reset" }
        endpoints.MapPost("bypass", SetBypass);          // { "bypass": true }  true = pass-through
        endpoints.MapPost("network", SetNetwork);        // { "network": "LC|CL" }
        endpoints.MapPost("lc", AdjustLc);               // { "deltaL": -1, "deltaC": +1 }
        endpoints.MapPost("memory/select", MemorySelect);// { "slot": 3 }
        endpoints.MapPost("memory/save", MemorySave);
        endpoints.MapPost("memory/reset", MemoryReset);  // undo: re-apply current slot
        endpoints.MapPost("sync", Sync);                 // refresh full state (incl. memory list)
    ...
    private IResult GetStatus()
    {
        ...
        var dto = conn.GetStatus() with { AutoTune = _autoTune };
    ...
    private async Task<IResult> MemorySave()
    {
        if (_conn is not { } c) return NotConnected();
        await c.MemorySaveAsync();
        return Results.Ok(c.GetStatus() with { AutoTune = _autoTune });
    }
...
internal sealed class Atr1000Connection : IAsyncDisposable
{
    ...
    public async Task MemorySaveAsync()
    {
        await SendAsync(Frame(MEMORY_ACTION, 1));
        await SendAsync(Frame(SYNC)); // refresh the memory list after a save
    }
